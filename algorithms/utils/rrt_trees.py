from typing import List, Tuple, Optional

import numpy as np
import matplotlib.pyplot as plt
from scipy.spatial import KDTree

from utils.types import RobotState
from algorithms.utils.rrt_nodes import RrtNode, RrtStarNode


class RrtTree:
    def __init__(self, config):
        self.config = config
        self.nodes: List[RrtNode] = []
        self.coords_to_idx_map = {}

        self.kd_tree: KDTree = None

    def reset_tree(self):
        self.nodes: List[RrtNode] = []
        self.coords_to_idx_map = {}

        self.kd_tree: KDTree = None

    def add_node(self, node_pos: Tuple[float, float], parent_idx: int) -> int:
        idx = len(self.nodes)
        node = RrtNode(idx, node_pos, parent_idx)
        self.nodes.append(node)
        if parent_idx is not None:
            self.nodes[parent_idx].add_child(idx)
        self.coords_to_idx_map[node_pos] = idx
        self.kd_tree = KDTree([node.get_pos() for node in self.nodes])
        return idx

    def add_root(self, root_pos):
        self.add_node(root_pos, None)

    def get_pos_of_node(self, idx: int) -> Tuple[float, float]:
        return self.nodes[idx].get_pos()

    def find_nearest_node(self, pos: Tuple[float, float]) -> RrtNode:
        distances, nearest_node_idx = self.kd_tree.query(np.array([pos]))
        return self.nodes[nearest_node_idx[0]]

    def check_if_pos_in_tree(self, pos: Tuple[float, float]) -> bool:
        return pos in self.coords_to_idx_map

    def get_idx_from_pos(self, pos: Tuple[float, float]) -> Optional[int]:
        if pos not in self.coords_to_idx_map:
            return -1
        return self.coords_to_idx_map[pos]

    def get_num_nodes(self):
        return len(self.nodes)

    def get_path_from_tree(
        self, goal: Tuple[float, float]
    ) -> Optional[List[Tuple[float, float]]]:
        if goal not in self.coords_to_idx_map:
            return None
        path = [goal]
        goal_idx = self.coords_to_idx_map[goal]
        node_on_path_idx = self.nodes[goal_idx].get_parent()
        while node_on_path_idx is not None:
            path.append(self.nodes[node_on_path_idx].get_pos())
            node_on_path_idx = self.nodes[node_on_path_idx].get_parent()

        return path[::-1]

    def get_path_idx_from_tree(self, goal: Tuple[float, float]) -> Optional[List[int]]:
        if goal not in self.coords_to_idx_map:
            return []
        goal_idx = self.coords_to_idx_map[goal]
        path = [goal_idx]
        node_on_path_idx = self.nodes[goal_idx].get_parent()
        while node_on_path_idx is not None:
            path.append(node_on_path_idx)
            node_on_path_idx = self.nodes[node_on_path_idx].get_parent()

        return path[::-1]


class RrtStarTree(RrtTree):
    def __init__(self, config):
        self.config = config
        self.near_node_dist_trheshold = config["near_node_dist_trheshold"]

        self.nodes: List[RrtStarNode] = []
        self.coords_to_idx_map = {}

        self.kd_tree: KDTree = None

    def reset_tree(self):
        self.nodes: List[RrtStarNode] = []
        self.coords_to_idx_map = {}

        self.kd_tree: KDTree = None

    def add_node(
        self,
        node_pos: Tuple[float, float],
        parent_idx: int,
        cost: float,
        cost_from_parent: float,
    ) -> int:
        idx = len(self.nodes)
        node = RrtStarNode(idx, node_pos, parent_idx, cost, cost_from_parent)
        self.nodes.append(node)
        if parent_idx is not None:
            self.nodes[parent_idx].add_child(idx)
        self.coords_to_idx_map[node_pos] = idx
        self.kd_tree = KDTree([node.get_pos() for node in self.nodes])
        return idx

    def add_root(self, root_pos) -> int:
        return self.add_node(root_pos, None, 0, 0)

    def propagate_cost_to_children(self, node_idx: int):
        for child_idx in self.nodes[node_idx].children:
            updated_cost = (
                self.nodes[node_idx].get_cost()
                + self.nodes[child_idx].get_cost_from_parent()
            )

            self.nodes[child_idx].update_cost(updated_cost)
            self.propagate_cost_to_children(child_idx)

    def find_near_nodes(self, new_node_pos: Tuple[float, float]) -> List[int]:
        near_nodes_idx = self.kd_tree.query_ball_point(
            np.array([new_node_pos]), self.near_node_dist_trheshold
        )
        return near_nodes_idx[0]

    def get_cost_of_node_pos(self, pos: Tuple[float, float]) -> float:
        if pos not in self.coords_to_idx_map:
            return float("inf")
        return self.nodes[self.coords_to_idx_map[pos]].get_cost()

    def get_cost_of_node_idx(self, idx: int) -> float:
        return self.nodes[idx].get_cost()

    def update_parent_and_cost(
        self,
        node_idx: int,
        new_parent_idx: int,
        new_cost: float,
        new_cost_from_parent: float,
    ):
        original_parent_idx = self.nodes[node_idx].get_parent()
        self.nodes[original_parent_idx].remove_child(node_idx)
        self.nodes[node_idx].update_parent_and_cost(
            new_parent_idx, new_cost, new_cost_from_parent
        )
        self.nodes[new_parent_idx].add_child(node_idx)

    def find_optimal_parent_and_add_node_to_tree(
        self,
        new_node_pos: Tuple[float, float],
        collision_free_near_nodes_idx: List[int],
        cost_to_collision_free_near_nodes: List[float],
    ) -> Tuple[int, float]:
        optimal_parent = None
        min_cost = float("inf")
        cost_to_parent = None
        for iter_idx, near_node_idx in enumerate(collision_free_near_nodes_idx):
            cost_via_near_node = (
                self.nodes[near_node_idx].get_cost()
                + cost_to_collision_free_near_nodes[iter_idx]
            )
            if cost_via_near_node < min_cost:
                min_cost = cost_via_near_node
                cost_to_parent = cost_to_collision_free_near_nodes[iter_idx]
                optimal_parent = near_node_idx

        if optimal_parent is None:
            raise ValueError("No optimal parent found")

        return self.add_node(new_node_pos, optimal_parent, min_cost, cost_to_parent)
